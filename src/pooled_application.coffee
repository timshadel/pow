# The `PooledApplication` class is responsible for commanding a
# process pool for a given application. Incoming HTTP requests
# are dispatched to a `PooledApplication` subclass instance by
# an `HttpServer`, where they are subsequently handled by the
# pool chosen by the specific subclass. By default, Pow dictates
# that the pool class use a maximum of two worker processes per
# application, but this can be overridden with the configuration's
# `workers` option.
#
# Before creating the process pool, Pow executes the `.powrc` and
...
# pooled worker processes. This lets you modify any environment var,
# for example you may set `RUBYOPT` to use different Ruby options.
#
# If [rvm](http://rvm.beginrescueend.com/) is installed and an
# `.rvmrc` file is present in the application's root, Pow will load
# both before creating the process pool. This makes it easy to run an
# app with a specific version of Ruby.
#
# Pooled workers remain running until they're killed, restarted (by
...
module.exports = class PooledApplication
  ...
  initialize: ->
    ...
    @loadEnvironment (err, env) =>
      ...
      # Set the application's state to ready. Then create the process
